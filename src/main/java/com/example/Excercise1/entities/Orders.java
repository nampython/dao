...
public class Orders implements ValueObject {
    ...
    public String getExecuteSql() {
        ...
        switch (this.resultCode) {
            case INSERT_CODE: {
                sqlQuery = ProcessDatabase.generatedSqlQuery().get("orders").get(3);
            }
            case UPDATE_CODE: {
                sqlQuery = ProcessDatabase.generatedSqlQuery().get("orders").get(2);

            }
            case DELETE_CODE: {
                sqlQuery = ProcessDatabase.generatedSqlQuery().get("orders").get(1);
            }
        }
        return sqlQuery;
    }
    ...
    public String getSelectSql() {
        return ProcessDatabase.generatedSqlQuery().get("orders").get(1);
    }

    @Override
    public String getDeleteSql() {
        return ProcessDatabase.generatedSqlQuery().get("orders").get(2);
    }

    @Override
    public String getUpdateSql() {
        return ProcessDatabase.generatedSqlQuery().get("orders").get(3);
    }
